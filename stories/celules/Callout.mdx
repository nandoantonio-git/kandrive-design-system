...
Aviso de destaque (âmbar/aviso ou teal/informativo) usado em contextos de
risco (ex.: exclusão de conta) ou informação relevante. Use quando precisar
interromper a atenção do usuário com um aviso contextual — pra mensagens de
rotina/confirmação use `Notification`, não este componente.
...
fresco no nó `1421:20028` + screenshot real confirmam ambas variantes
(`Warning`/`Info`) pixel-a-pixel batendo com o Figma — sem divergência
nova.
...
[`organism/FAQ/info/Card`](../organisms/FaqInfoCard.mdx) (`FaqCallout`,
