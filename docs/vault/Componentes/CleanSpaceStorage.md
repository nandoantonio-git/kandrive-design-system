...
Duas seções: "Arquivos grandes" (lista via `CleanSpaceListSelection` (molécula) — sem nota própria no vault, ver [[Camadas Atômicas]] e `stories/molecules/CleanSpaceListSelection.mdx`, reusada — Regra 10) e "Arquivos duplicados" (prévia ilustrativa, com badge "Prévia" indicando que a detecção de fato ainda não existe).
