...
- **Radius do modo coluna (2026-09-25).** Achado do usuário: "borda padrão de arredondamento no modo coluna, a página não reflete o Figma". Investigado no código — `FileListContainer` (8px, cantos esquerdos) e `PreviewPane` (20px, todos os cantos) batem com a escala de radius documentada (`Tokens/Spacing`, elemento maior = raio maior), sem bug óbvio só na leitura. Fica pendente até o usuário trazer um print ou link do Figma da tela real.
