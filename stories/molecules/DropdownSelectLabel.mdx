...
[`DropdownSelectGroupBy`](./DropdownSelectGroupBy.mdx) (não confundir os
dois, conforme nota da US-022). Estruturalmente quase idêntico a
`molecule/Label` (mesmo node-pattern no Figma) — use este quando o contexto
for especificamente o canvas de organização, `molecule/Label` nos demais
fluxos de etiquetagem.

**Status: ✅ aligned (estado fechado)** — protocolo completo da Regra 11
...
**Reverificado em auditoria de ponto-fixo (3ª passada, 2026-08-12):** novo
`get_design_context` no nó `1439:19650` retorna byte-a-byte o mesmo código
