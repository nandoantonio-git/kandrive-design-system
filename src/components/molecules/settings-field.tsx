...
function SettingsField({ label, className, ...props }: SettingsFieldProps) {
  ...
  return (
    <div className="flex min-w-px flex-1 flex-col gap-1.5">
      <label htmlFor={inputId} className="text-[0.8125rem] font-medium text-zinc-950">
        {label}
      </label>
      <input
        id={inputId}
        className={cn(
          "h-9 w-full rounded-md border border-[#d4d4d4] bg-effect-glass-white-70 px-3 text-sm text-zinc-950",
